...
unsigned char parseCommand(unsigned char byte){
    /*
    //gross bit manipulations...
    // a struct with bitfields might be nicer, but i haven't tested it
    // for example
    stuct cmd{
     unsigned char action:2;
     unsigned char player_id:5;
     };
     This should be check in the compiler docs to see how the bitfield is populated.
     It might required some special things like __attr__(__Packed__))__????WTF
     */
...
/*
 // Wonderful state machine that was never used because it was overly complicated
 // for the players, and also to understand
    switch(command){
        case COMMAND_HIT:
